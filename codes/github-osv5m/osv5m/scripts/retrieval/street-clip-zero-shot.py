...
if __name__ == "__main__":
    ...
    default_features_parent = Path(
        os.environ.get("OSV5M_FEATURES_DIR", project_root / "data" / "street-clip")
    )
    default_data_parent = Path(
        os.environ.get("OSV5M_RETRIEVAL_DATA_DIR", project_root / "data" / "osv5m")
    )
    ...
    parser.add_argument(
        "--features_parent", type=str, default=str(default_features_parent)
    )
    parser.add_argument(
        "--data_parent", type=str, default=str(default_data_parent)
    )
    ...
    indices_path = Path(args.features_parent) / "test3_indices.txt"
